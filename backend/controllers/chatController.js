  ...
  async sendMessage(req, res) {
    ...
    if (this.processingMessages.has(chatId)) {
      return res.status(429).json({ 
        message: 'Message is already being processed for this chat' 
      });
    }

    try {
      // Add chat to processing set
      ...
      // Check if the last message is from the user to prevent duplicate processing
      const lastMessage = chat.messages[chat.messages.length - 1];
      if (lastMessage && lastMessage.role === 'user' && lastMessage.content === req.body.message) {
        this.processingMessages.delete(chatId);
      ...
      try {
        // Get AI response
        const aiResponse = await this.getGiminAIResponse(chat.messages);
        ...
        // Emit the new message through WebSocket only once
        if (this.io) {
          this.io.to(chatId.toString()).emit('message-received', {
            chatId: chat._id,
        ...
        // If AI response fails, remove the user message
        chat.messages.pop();
        await chat.save();
        throw aiError;
      }
      ...
      // Always remove the chat from processing set
      this.processingMessages.delete(chatId);
    }
  }
